...
import { JsonFormat, JsonMinify, JsonValidate } from '@/utils/wailsApi';

/**
 * JsonTool — JSON formatting tool with 3 actions:
...
const JsonTool: React.FC = () => {
  ...
  const { call: formatCall } = useGoMethod('JsonFormat', JsonFormat);
  const { call: minifyCall } = useGoMethod('JsonMinify', JsonMinify);
  const { call: validateCall } = useGoMethod('JsonValidate', JsonValidate);
